...
export function getPrData(pr: PullRequest): PrData | undefined {
  ...
  const match = content.match(/## vcpkg-fmt-template@(?<version>\d.+)/);
  ...
  const body = [
    ...
    `description: Explore the changelog for vcpkg-fmt-template version ${version}. Learn about the latest features, bug fixes, and improvements.`,
...
export async function getMergedPrs(): Promise<PullRequests> {
  const { data } = await octokit.pulls.list({
    ...
    repo: "vcpkg-fmt-template",
    base: "main",
    head: "vcpkg-fmt-template:changeset-release/main",
    per_page: 100,
  });
